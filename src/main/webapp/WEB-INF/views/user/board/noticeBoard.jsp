			...
			<div class="search-form">
				<form id="search-btn" action="/board/noticeBoard" method="get">
					<select name="type">
		                ...
		                <option value="W" <c:out value="${pageMaker.cri.type eq 'W'?'selected':'' }"/>>관리자</option>
					</select>
		...
		<div>
			<table class=notice-table>
				...
				<c:forEach var="list" items="${list }"> 
					<tr>
						<td>${list.no }</td>
						<td class=td-tag><a href='/board/noticeBoard/get?no=${list.no}'>${list.title }</a></td>
						<td>${list.adminid }</td>
			...
			<!-- 페이지네이션 -->
                  	<div class="pagination-container">
                    	...
                    	<c:if test="${canEnrol }">
							<a class="enrol-box" href="/board/noticeBoard/enroll">글쓰기</a>
                    	</c:if>
                    </div>
                    <!--end 페이지네이션 -->
		</div>
